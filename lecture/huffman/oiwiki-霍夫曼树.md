# 霍夫曼树（OI-Wiki 原文存档）
> **来源**：OI-Wiki《霍夫曼树》 <https://oi-wiki.org/ds/huffman-tree/>
> **本机位置**：`OI-wiki/ds/huffman-tree/index.html`（构建产物）；本文件由同目录 `extract_oiwiki.py` 抽取，
> 公式已从 MathJax 渲染结果还原为 LaTeX，正文未作改动（只去掉站点导航、代码行号与页脚）。
> **许可**：内容来自 OI-Wiki，按 **CC BY-SA 4.0** 协议提供（附加 SATA 条款）；配图见 `oiwiki/` 目录。
> **贡献者**：Alex-McAvoy、1Wizzy、Ir1d、ksyx、StudyingFather（页面最近更新 2023/2/18）。
> 再分发本页内容时请保留这段署名与协议说明。
